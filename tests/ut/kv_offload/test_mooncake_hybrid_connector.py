...
class TestMooncakeHybridConnectorWorkerSnapshotRestore(unittest.TestCase):
    def setUp(self):
        for patcher in (
...
class TestMooncakeHybridConnectorWorker(unittest.TestCase):
    def test_rebuild_stops_listener_and_destroys_old_engine_before_recreate(self):
        ...
        with (
            ...
            patch("vllm_ascend.distributed.kv_transfer.kv_p2p.mooncake_hybrid_connector.get_tp_group"),
            patch("vllm_ascend.distributed.kv_transfer.kv_p2p.mooncake_hybrid_connector.threading.Event"),
        ):
