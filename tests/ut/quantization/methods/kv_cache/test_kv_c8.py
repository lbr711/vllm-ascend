...
class TestAscendFAQuantAttentionMethodProcessWeights(unittest.TestCase):
    ...
    def test_process_weights_restores_derived_scales(self):
        ...
        self.assertIn("quant_kscale", self.layer.state_dict())
        self.assertIn("fak_descale_reciprocal", self.layer.state_dict())
    ...
    def test_snapshot_disabled_does_not_register_derived_scales(self):
        self.mock_config.snapshot_config = None
        self.layer.fa_k.scale = nn.Parameter(torch.tensor([[2.0]]), requires_grad=False)
        self.layer.fa_k.offset = nn.Parameter(torch.tensor([[0]], dtype=torch.int8), requires_grad=False)
        self.method.process_weights_after_loading(self.layer)
        self.assertNotIn("quant_kscale", self.layer.state_dict())
        self.assertNotIn("fak_descale_reciprocal", self.layer.state_dict())
