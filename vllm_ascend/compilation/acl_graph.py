...
def clear_all_aclgraph_entries():
    """Clear captured ACL graphs from all registered ACLGraphWrapper instances.
    This must be called before recapturing graphs (e.g. after snapshot restore)
    so that capture_model will trigger fresh captures instead of replaying
    stale graphs.

    Each NPUGraph is explicitly reset before dropping the reference so that
    its C++ resources (and the associated graph-pool allocations) are released
    deterministically rather than relying on the garbage collector.
    """
    import gc

    # Ensure all pending NPU work finishes before touching graph objects.
    try:
...
def _reset_params(params: GraphParams | None) -> None:
    """Clear all per-size entries in *params* so capture_model can repopulate."""
    if params is None:
        return
