...
def dump_state_dict(model: nn.Module, path: str) -> None:
    ...
    logger.info(
        "[snapshot][checkpoint] memory usage: phase=after_npu_cache_clear rss=%.2f MiB",
        process.memory_info().rss / 1024**2,
    )
    try:
...
def restore_state_dict(model: nn.Module, path: str, label: str) -> None:
    ...
    logger.info(
        "[snapshot][checkpoint] checkpoint loaded: model=%s path=%s tensors=%d duration=%.4f s",
        label,
        path,
        len(state_dict),
        time.time() - start,
    ...
    logger.info(
        "[snapshot][checkpoint] restore completed: model=%s path=%s duration=%.4f s",
        label,
        path,
        time.time() - start,
    )
